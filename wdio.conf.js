...
exports.config = {
    ...
    //
    maxInstances: SAUCE_USERNAME && SAUCE_ACCESS_KEY ? SAUSE_MAX_SESSIONS : LOCAL_MAX_SESSIONS,
    //
    ...
    onPrepare: async (config, capabilities) => {
      //bug in wdio config -- suite cloned twice
      const suite = [...new Set(config.suite)];
      await sheetData.sheets(suite);

      if (capabilities[0]["custom:params"].runPixDiffSuite = suite.includes("gdrive")) {
    ...
    before: async (capabilities, specs, browser) => {
      await browser.setTimeouts(1000, 60000, 60000);
      if (!(SAUCE_USERNAME && SAUCE_ACCESS_KEY)) await browser.setWindowRect(0, 0, screenSize.width, screenSize.height);
  
      global.locators = require("./e2e/pageObjects/pageObjects.js");
  
      // if (config.capabilities.device) {
      //   Object.assign(config.params, ["mobile", "tablet", "desktop"].reduce((res, device) => {
      //     res[device] = device === config.capabilities.device;
      //     return res;
      //   }, {}));
      // }

      if (capabilities["custom:params"].runPixDiffSuite) {
    ...
    // },
    onComplete: async (exitCode, config, capabilities) => {
      if (capabilities[0]["custom:params"].runPixDiffSuite) {
